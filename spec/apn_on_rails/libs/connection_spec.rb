...
describe APN::Connection do
  ...
  describe 'open_for_delivery' do
    ...
    context "overridden defaults" do
      before do
        stub_const('APN_CONFIG', {})
        APN_CONFIG[:cert] = File.expand_path(File.join(File.dirname(__FILE__), '..', '..', 'fixtures', 'fake_cert.bin'))
        APN_CONFIG[:passphrase] = 'secr3t'
        APN_CONFIG[:host] = 'www.example.com'
        APN_CONFIG[:port] = 9999
      end
      it 'should create a connection to Apple, yield it, and then close' do
        rsa_mock = mock('rsa_mock')
        OpenSSL::PKey::RSA.should_receive(:new).with('fakecert', 'secr3t').and_return(rsa_mock)
        ...
        tcp_mock = mock('tcp_mock')
        tcp_mock.should_receive(:close)
        TCPSocket.should_receive(:new).with('www.example.com', 9999).and_return(tcp_mock)
    ...
    context "with options passed as arguments" do
      let(:apn_config_options) do
        {
          :cert => File.expand_path(File.join(File.dirname(__FILE__), '..', '..', 'fixtures', 'fake_cert.bin')),
          :passphrase => 'fooz',
      ...
      it 'should create a connection to Apple, yield it, and then close' do
        rsa_mock = mock('rsa_mock')
        OpenSSL::PKey::RSA.should_receive(:new).with('fakecert', 'fooz').and_return(rsa_mock)
        ...
        tcp_mock = mock('tcp_mock')
        tcp_mock.should_receive(:close)
        TCPSocket.should_receive(:new).with('apple.example.com', 9998).and_return(tcp_mock)
        ...
        APN::Connection.open_for_delivery(apn_config_options) do |conn, sock|
          conn.write('message-0')
          conn.write('message-1')
        end
      end
