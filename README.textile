...
AN on Rails has the following default configurations that you change as you
see fit:
...
<pre><code>
  ...
  APN_FEEDBACK_CONFIG[:cert] #=> File.join(RAILS_ROOT, 'config', 'apple_push_notification_development.pem')
  
  # production (feedback):
  APN_FEEDBACK_CONFIG[:host] # => 'feedback.push.apple.com'
  APN_FEEDBACK_CONFIG[:cert] #=> File.join(RAILS_ROOT, 'config', 'apple_push_notification_production.pem')
</code></pre>
